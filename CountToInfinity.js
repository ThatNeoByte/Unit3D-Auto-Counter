...
(function () {
    ...
    function createBonPoolMessage(match) {
        const contributionAmount = match[1].replace(',', ''); // Remove commas from the contribution amount
        const percentage = match[2];
        const newMessage = `Contributed ${contributionAmount} to the Bon Pool, the pool is now at ${percentage}%`;
        return newMessage;
    }
    ...
    const HANDLERS = [
        ...
        {
            // Format: [BON-POOL]-[Contribution: amount bon]-[Progress: percentage %]-[By: Nickname]-[Contribute here: https://darkpeers.org/bon-pool]
            // Used at: DP
    ...
    // Returns { username, modifyContent, prefixToRemove, metadata, newMessage } or null if no match
    ...
    function processMessage(messageElement) {
        ...
        let { username, modifyContent, prefixToRemove, metadata, newMessage } = parsed;
